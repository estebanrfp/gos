---
name: things-mac
description: Manage Things 3 via the things CLI on macOS (add, search, list tasks and projects).
...
Use when a user asks to add a task to Things, list inbox/today/upcoming, search tasks, or inspect projects/areas/tags. Supports add/update projects+todos via URL scheme and read/search/list from the local Things database.
...
- If DB reads fail: grant **Full Disk Access** to the calling app (Terminal for manual runs; `NyxClaw.app` for gateway runs).
