---
name: bluebubbles
description: Send and manage iMessages via BlueBubbles integration.
metadata: { "nyxclaw": { "emoji": "🫧", "requires": { "config": ["channels.bluebubbles"] } } }
---

# BlueBubbles Actions

Recommended iMessage integration. Calls go through the generic message tool with `channel="bluebubbles"`.

## Overview

BlueBubbles is NyxClaw’s recommended iMessage integration. Use the `message` tool with `channel: "bluebubbles"` to send messages and manage iMessage conversations: send texts and attachments, react (tapbacks), edit/unsend, reply in threads, and manage group participants/names/icons.
